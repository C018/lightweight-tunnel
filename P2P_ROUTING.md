...
| 连接类型 | 底层传输 | 伪装方式 | 说明 |
|---------|---------|---------|------|
| 客户端-服务器 | UDP | 带伪装TCP头 | 使用 UDP 传输，添加假的 TCP 头部绕过简单防火墙 |
| P2P 直连 | 纯 UDP | 无伪装 | 客户端之间使用纯 UDP 连接，无 TCP 伪装 |

### 与 udp2raw 的对比

本项目借鉴了 **udp2raw** 的思想，但实现方式不同：

| 特性 | lightweight-tunnel | udp2raw |
|------|-------------------|---------|
| 传输层 | UDP | UDP |
| TCP 伪装方式 | 应用层添加 TCP 头 | raw socket 完全伪装 |
| P2P 流量 | 纯 UDP（无伪装） | 不支持 P2P |
| DPI 抵抗 | 较弱 | 较强 |
| 系统权限 | 普通 root | CAP_NET_RAW |

**重点说明：**
- 客户端与服务器之间的隧道流量：UDP + 伪装 TCP 头部
- P2P 直连流量：纯 UDP（无 TCP 伪装）
- 如果 P2P 环境的防火墙限制较严，可能需要禁用 P2P 或配置防火墙规则

### 本地网络优先

从本版本开始，P2P 连接会优先尝试使用本地网络地址（内网 IP）直连：

1. **首先尝试**：本地/内网 IP 地址（等待 2 秒）
2. **如果失败**：回退到公网 IP 地址进行 NAT 穿透
3. **如果都失败**：使用服务器中转

这确保了同一局域网内的客户端能获得最低延迟的直接连接。
